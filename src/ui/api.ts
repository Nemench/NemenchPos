...
export const api = {
  ...
  roles: {
    list: () => req<AppRole[]>("GET", "/roles"),
    create: (data: AppRoleInput) => req<AppRole>("POST", "/roles", data),
    update: (id: string, data: AppRoleInput) => req<AppRole>("PUT", `/roles/${id}`, data),
    remove: (id: string) => req<{ success: boolean }>("DELETE", `/roles/${id}`)
  },
  products: {
  ...
  printers: {
    ...
    queue: () => req<PrintQueueJob[]>("GET", "/printers/queue"),
    cancelJob: (id: string) => req<{ ok: boolean }>("DELETE", `/printers/queue/${encodeURIComponent(id)}`),
    cancelAll: () => req<{ ok: boolean; canceled: number }>("POST", "/printers/queue/cancel-all")
  },
