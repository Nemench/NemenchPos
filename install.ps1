...
<#
.SYNOPSIS
    NemenchPos — Windows Server Installer
.DESCRIPTION
    Installs NemenchPos as a persistent Windows service using NSSM on Windows 10/11.
...
.PARAMETER InstallDir
    Directory where the NemenchPos repo will be cloned. Default: C:\opt\nemenchpos
.PARAMETER Port
    HTTP port the server will listen on. Default: 3000
.PARAMETER ServiceName
    Name used to register the Windows service. Default: nemenchpos
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File install.ps1
    powershell -ExecutionPolicy Bypass -File install.ps1 -Port 8080 -InstallDir D:\nemenchpos
#>
[CmdletBinding()]
param(
...
# and makes manual troubleshooting easy (just run start-nemenchpos.cmd in a terminal).
#
...
$StartCmd = Join-Path $InstallDir "start-nemenchpos.cmd"
$batchContent = @"
@echo off
REM NemenchPos — service launcher (auto-generated by install.ps1)
...
# Register: Application = cmd.exe, AppParameters = /c "start-nemenchpos.cmd"
& $NssmExe install $ServiceName cmd.exe "/c `"$StartCmd`""
& $NssmExe set $ServiceName AppDirectory      $InstallDir
& $NssmExe set $ServiceName DisplayName       "NemenchPos"
& $NssmExe set $ServiceName Description       "NemenchPos kitchen order ticket server"
& $NssmExe set $ServiceName Start             SERVICE_AUTO_START
& $NssmExe set $ServiceName AppStdout         (Join-Path $LogDir "nemenchpos.log")
& $NssmExe set $ServiceName AppStderr         (Join-Path $LogDir "nemenchpos-error.log")
...
Write-Host "  NemenchPos is running!"                            -ForegroundColor Green
Write-Host ""
...
Write-Host "  Windows user account via Services (services.msc) > NemenchPos > Log On." -ForegroundColor Yellow
Write-Host ""
